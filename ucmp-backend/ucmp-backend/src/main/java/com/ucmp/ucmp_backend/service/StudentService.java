...
//    }
    ...
    public Optional<Student> findByCollegeId(String collegeId) {
    ...
    //get faculty for a student (via Section)

    public Set<Faculty> getFacultiesForStudent(Long studentId) {
        Student student = studentRepository.findById(studentId).orElseThrow(() -> new RuntimeException("Student not found"));

        if(student.getSection() == null){
            throw new RuntimeException("Student not assigned to any section");
        }
        return student.getSection().getFaculties();
    }
}
