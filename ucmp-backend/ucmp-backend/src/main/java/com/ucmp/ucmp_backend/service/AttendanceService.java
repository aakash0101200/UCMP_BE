...
public class AttendanceService {
    ...
    // ── Start Session ──────────────────────────────────────────────────────────
    /**
     * @param mergedSectionIds Additional section IDs to include (for MERGED
     *                         sessions).
     *                         null or empty = REGULAR session.
     *                         The primary sectionId is always included
     *                         automatically.
     */
    @Transactional
    public AttendanceSession startSession(Long facultyId, Long sectionId, Long subjectId,
        ...
        if (scheduledFacultyId != null) {
            ...
            try {
                ...
                List<TimetableEntryResponseDTO> dailySchedule = resolutionService
                        .getResolvedScheduleForSection(sectionId, today, "2026-27-ODD");
        ...
        // Idempotency check: if there's an active session for the same faculty,
        // section, and subject
        ...
        if (subjectId != null) {
            Optional<AttendanceSession> existingOpt = sessionRepository.findByFacultyIdAndIsActiveTrue(facultyId)
                    ...
                    .filter(s -> s.getSection().getId().equals(sectionId)
                            ...
                            && s.getStartTime()
                                    .isAfter(LocalDateTime.now().minusMinutes(
                                            s.getDurationInMinutes() != null ? s.getDurationInMinutes() : 40)))
                    .findFirst();
        ...
        try {
            ...
            for (AttendanceSessionSection ass : activeSections) {
                ...
                try {
                    ...
                    attendanceAnnouncement.setTitle("Attendance Session Started: "
                            + (saved.getSubject() != null ? saved.getSubject().getName() : "General Class"));
                    attendanceAnnouncement
                            .setDescription(String.format("Attendance marking is active for %s. Code: active",
                                    saved.getSubject() != null ? saved.getSubject().getName() : "General Class"));
                    attendanceAnnouncement
                            .setAuthor(faculty.getUser() != null ? faculty.getUser().getName() : "Faculty");
                    ...
                    messagingTemplate.convertAndSend("/topic/notifications/section/" + targetSectionId,
                            attendanceAnnouncement);
                } catch (Exception ex) {
    ...
    public Optional<AttendanceSession> findActiveSessionForStudent(String collegeId) {
        ...
        Optional<AttendanceSession> sessionOpt = sessionRepository
                .findActiveSessionForSection(student.getSection().getId());
    ...
    public void markAttendance(Long sessionId, Long studentId,
        ...
        // We allow current code, last 2 previous codes, and next code (to tolerate
        // clock drift, network lag, or server spin-up latency)
    ...
    public List<StudentAttendanceDTO> manualMarkAttendance(
        ...
        for (String collegeId : studentCollegeIds) {
            Student student = studentRepository.findByCollegeId(collegeId)
                    .orElse(null);
            if (student == null)
                continue; // Skip unknown college IDs silently
    ...
    public List<Map<String, Object>> getAbsentStudentsForSession(Long sessionId) {
        ...
        List<AttendanceSessionSection> sessionSections = sessionSectionRepository.findBySessionId(sessionId);
    ...
    public List<AttendanceSummaryDTO> getMyAttendanceSummary(String collegeId) {
        ...
        // Fetch all subject assignments for this student's section (deduplicated by
        // subjectId)
