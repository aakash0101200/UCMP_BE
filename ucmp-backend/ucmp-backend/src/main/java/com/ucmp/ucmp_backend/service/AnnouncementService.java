...
public class AnnouncementService {
    ...
    public List<Announcements> getAllForUser(Authentication authentication) {
        ...
        if (isAdmin) {
            boolean isSuper = "ADMIN_001".equals(collegeId) || user.getDepartment() == null || "Administration".equalsIgnoreCase(user.getDepartment());
            if (isSuper) {
                return all;
            }
            ...
            return all.stream()
                .filter(a -> {
                    if (collegeId.equals(a.getAuthor())) return true;
                    if (a.getTargetDept() != null && !a.getTargetDept().equalsIgnoreCase(dept)) return false;
                    if (a.getTargetYear() != null && !a.getTargetYear().equals(yearScope)) return false;
                    if (a.getSectionId() != null) {
                        ...
                        if (dept != null && !dept.equalsIgnoreCase(secDept)) return false;
                        if (yearScope != null && !yearScope.equals(secYear)) return false;
                    }
                    return true;
                })
                .toList();
            ...
            return all.stream()
                .filter(a -> {
                    if (collegeId.equals(a.getAuthor())) return true;
                    if (a.getStudentCollegeId() != null) return false;
                    if (a.getTargetRole() != null && !a.getTargetRole().equalsIgnoreCase("FACULTY")) return false;
                    if (a.getTargetDept() != null && dept != null && !a.getTargetDept().equalsIgnoreCase(dept)) return false;
                    return true;
                })
                .toList();
        } else if (isStudent) {
            Student student = studentRepository.findByCollegeId(collegeId).orElse(null);
            final Long sectionId = (student != null && student.getSection() != null) ? student.getSection().getId() : null;
            final Integer year = (student != null && student.getYear() != null) ? Integer.parseInt(student.getYear()) : null;
            final String dept = (student != null && student.getBatch() != null) ? student.getBatch().getBatchName() : null;
            
            return all.stream()
                .filter(a -> {
                    if (collegeId.equalsIgnoreCase(a.getStudentCollegeId())) return true;
                    if (sectionId != null && sectionId.equals(a.getSectionId())) return true;
                    if (a.getSectionId() == null && a.getStudentCollegeId() == null) {
                        if (a.getTargetRole() != null && !a.getTargetRole().equalsIgnoreCase("STUDENT")) return false;
                        if (a.getTargetYear() != null && !a.getTargetYear().equals(year)) return false;
                        if (a.getTargetDept() != null && dept != null && !a.getTargetDept().equalsIgnoreCase(dept)) return false;
                        return true;
                    }
                    return false;
